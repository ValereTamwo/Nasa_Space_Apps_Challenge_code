...
const ScatterPlot3D = ({ selectedContinent, selectedSocialIndex, selectedDisasterType }) => {
      ...
      fetch('http://localhost:5000/api/data')
        ...
        .then((data) => {
          ...
          const points = filteredData.map((item) => {
            ...
            return {
              position: [x, y, z],
              label: `${item.COUNTRY}: (${x.toFixed(2)}, ${y.toFixed(2)}, ${z.toFixed(2)})`, // Include X, Y, Z in label
  ...
  return (
    <div className='mt-8 w-full h-[100vh] bg-gray-900 top-[25px]'>
      <h3 className='text-white p-3 w-[400px]'>
        {selectedSocialIndex} vs Gender Inequality Index vs {selectedDisasterType}
      </h3>
